...
if ($BUILD_CACHED) {
    ...
    ./scripts/quartus/copy-cached.ps1 $BUILD_FOLDER
}
else {
    ./scripts/quartus/build.ps1
}
...
$programmer_process.Arguments = "-File $SP/quartus/program.ps1" #process arguments
...
### here I'm using the dcache, icache, and test case I was looping over in the example scripts
...
while ($LINE -ne "---START $dcache $icache $i---" ) {
    $LINE = $terminal.StandardOutput.ReadLine()
}
...
./scripts/quartus/save.ps1 $OUT_FOLDER
...
if (!$BUILD_CACHED) {
    ...
    ./scripts/quartus/cache.ps1 $BUILD_FOLDER
}
